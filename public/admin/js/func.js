...
var fulltext = '';
function getFullName(tree,parentid){
    if(parentid == 0) return;
    var p =  tree.find("li[data-id='"+parentid+"']");
    if(p){
        fulltext = p.children('a.jstree-anchor').text() + fulltext;
        if(parentid>0){
            getFullName(tree,p.closest(".jstree-node").attr("data-parentid"));
        }
    }
}/** 
 * Jtree转html
*/
jQuery.jstreeList= function($lists){
    $html = '';
    $.each($lists,function($i,$o){
        if($o['childs']!==undefined){
            $html += "<li data-id="+$o['id']+" data-parentid="+$o['parentid']+" is_upload="+$o['is_upload']+">"+$o['name']+"<ul>";
            $html +=jQuery.jstreeList($o['childs']);
            $html +=  "</ul></li>"; 
        }else{
            $html +="<li data-id="+$o['id']+" data-parentid="+$o['parentid']+" is_upload="+$o['is_upload']+" data-jstree='{\"type\":\""+$o['name']+"\"}'>"+$o['name']+"</li>";
        }
 ...
 * Jtree 方法
*/
jQuery.fn.chooseJtree = function(ajax,name,succ){
        var url  ="index.php?_url=admin/privilege/choosearea&name="+name;
        layer.open({
            type: 2,
            fixed: false, //不固定
            maxmin: true,
            ...
            yes: function(index, layero) {
                var Tree = $(layero).find("iframe")[0].contentWindow.document.getElementById("using_json");
                ...
                parent.$("input[name="+name+"_text]").val(fulltext);
                ajax.get("index.php?_url=admin/device/getlatitude_longitude&area_id="+area_id,function(res){
                    if(res.status=="ok"){
                        parent.$("input[name=mappint]").val(res.message[0]);
                    }
                })
                parent.$("input[name="+name+"]").val(area_id).change();
                layer.close(index);
            },
